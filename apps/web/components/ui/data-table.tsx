...
interface DataTableProps<T> {
  ...
  /**
   * Caps the table's height and makes the row body scroll internally, with
   * the header pinned, once rows overflow it — e.g. `"320px"`. Use this
   * whenever the table shares a row with a fixed-purpose sibling card (a
   * dashboard KPI/chart card) so real-world row counts can't blow the whole
   * card out of proportion. Leave unset for a dedicated list page, where the
   * page itself is the right thing to scroll.
   */
  maxHeight?: string;
}

export function DataTable<T>({ columns, rows, rowKey, loading, empty, onRowClick, maxHeight }: DataTableProps<T>) {
    ...
    <div className="overflow-x-auto">
      <div className={cn(maxHeight && 'overflow-y-auto')} style={maxHeight ? { maxHeight } : undefined}>
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className={cn('border-b border-border-divider', maxHeight && 'sticky top-0 z-10 bg-bg-elevated')}>
              {columns.map((col) => {
