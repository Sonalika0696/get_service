...
- **Comprehensive seed (landed as scripts — rows are REFRESHABLE):** the shared dev DB is volatile (other sessions wipe/reseed it, and it periodically drops to 0 rows), so the durable artifact is the idempotent script, not the live rows. To restore the full realistic dataset any time: `cd backend && node scripts/reset-db.mjs` (TRUNCATEs every app table, keeps `_prisma_migrations`) `&& node scripts/seed-comprehensive.mjs` (drives the REAL flows via signed Razorpay-webhook stubs + the approval ladder so money/audit state is consistent; refuses to run unless RAZORPAY/SMS stubs are active; ends with a cross-society `assertAllBalances` integrity check). After a baseline reseed that predates 9.1, run the operator `POST /operator/societies/:sid/virtual-accounts/backfill` to provision any flats missing a VirtualAccount. Latency benchmark harness + numbers: `backend/scripts/realtime-bench.mjs`, `scripts/realtime-latency-report.md`.
